...
def oks_p9_create(request):
    ...
    return render(request, 'asrmb_oks/forms/oks_p9/form_create.html', context)


def oks_p9_edit(request, date_oks_p9):
    oks_p9_items = P9ComponentOfTheSeparationGas.objects.filter(
        date_create__contains=date_oks_p9).order_by('date_update')[:12]

    if not oks_p9_items:
        raise Http404("Нет данных")

    if request.method == 'POST':
        form_set = P9ModelFormSet(request.POST)
        print('Форма валидна: ' + str(form_set.is_valid()))
        if form_set.is_valid():
            for form in form_set:
                form.save()
        ...
        form_set = P9ModelFormSet(queryset=oks_p9_items)
        for f in form_set:
            print(f.as_table())

    context = {
        'just_day': date_oks_p9,
        'form_set': form_set,
    }
    return render(request, 'asrmb_oks/forms/oks_p9/form_edit.html', context)


def oks_p9_delete(request, date_oks_p9):
    oks_p9_items = P9ComponentOfTheSeparationGas.objects.filter(
        date_create__contains=date_oks_p9).order_by('date_update')[:12].values_list('id', flat=True)
    print(oks_p9_items)
    ...
    if request.method == 'POST':
        P9ComponentOfTheSeparationGas.objects.filter(pk__in=list(oks_p9_items)).delete()
        return redirect('oks_p9')
